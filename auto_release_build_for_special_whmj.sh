...
PROJ_ANDROID_PATH=src/game_majiang_client/proj.android
PROJ_WIN32_PATH=src/game_majiang_client/proj.win32
...
COPY_FILE()
{
    ...
    payment_config=`cat ${ROOT_PATH}/${PROJECT_NAME}/$PROJ_ANDROID_PATH/assets/game.properties`
...
BUILD_APK_PATH()
{
	...
	TEMP_VAR=""
    TEMP_VAR=`cat $PROJ_ANDROID_PATH/assets/VERSION`
    TEMP_VAR="$RUNNING_DATE"
    RESULT_PATH="$TEMP_VAR"

    TEMP_VAR=`cat $PROJ_ANDROID_PATH/AndroidManifest.xml | \
...
RESET_PROJECT()
{
    ...
    cd $PROJ_ANDROID_PATH
	echo $PROJ_ANDROID_PATH
...
if [ ! -f "$PROJECT_NAME/src/game_majiang_client/build_version/build_version.sh" ]; then
...
for (( i = 1; i <= LINE_NUMBER; i++));do
    ...
    echo "build version"
    cd $PROJECT_NAME/src/game_majiang_client
    ...
    cd $ROOT_PATH/$PROJECT_NAME/src/game_majiang_client/build_version
    ...
    echo "build release"
        currentPath=`pwd`
    cd $ROOT_PATH/$PROJECT_NAME/$PROJ_ANDROID_PATH/assets
